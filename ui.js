...
async function updateLogListView(isAppend = false) {
    ...
    try {
        ...
        // 描画実行
        renderLogList(newLogs, isAppend);
...
function renderBeerTank(currentBalanceKcal) {
    ...
    const { 
        ...
        liquidColor,
        isHazy 
    } = Calc.getTankDisplayData(currentBalanceKcal, StateManager.beerMode);
    ...
    const msgContainer = document.getElementById('tank-message');
    ...
    requestAnimationFrame(() => {
        ...
        if (currentBalanceKcal > 0) { // 貯金あり (kcal > 0)
            ...
            if (debtCansVal > 1.5) {
                // 1缶分を消費するのに必要な時間
                const oneCanMin = Calc.convertKcalToMinutes(unitKcal, Store.getBaseExercise());
...
// --- ui.js (Part 2/3) ---
// 前回の renderBeerTank の直後に続けてください
...
export const UI = {
    // ... initDOM は Part 1 で定義済みですが、UIオブジェクトのメソッドとして追加していきます
    // ここでは個別のモーダル制御関数などをエクスポートします

    // 注意: initDOM自体は Part 1 の冒頭で DOM オブジェクトへの参照として準備されていますが、
    // ここで UI オブジェクトに紐づける形でメソッドを定義します。
    // Part 1 の DOM 変数と連携します。
...
// --- ui.js (Part 3/4) ---
// 前回の openCheckModal の直後に続けてください
